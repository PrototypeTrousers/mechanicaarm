...
import colladamodel.client.model.*;
...
import colladamodel.client.model.transform.*;
...
public class ColladaAsset {
    ...
    public String zAxis;
    public String yAxis;
    ModelLoaderRegistry.LoaderException e;
    ...
    private Collection<Element> getXPathNodeListRecursively(Element node, String path) throws ModelLoaderRegistry.LoaderException {
        try {
    ...
    private Model parseScene(Element sceneElem) throws ModelLoaderRegistry.LoaderException {
        ...
        Map<Node, Transformable> nodeTransformableMap = new Object2ObjectOpenHashMap<>();
        for (Element nodeElem : getXPathNodeListRecursively(sceneElem, "node")) {
            parseSceneNode(model, nodeElem,nodeTransformableMap);
        }
        return model;
    }

    private void parseSceneNode(Model model, Element nodeElem, Map<Node, Transformable> nodeTransformableMap) {
        ...
        try {
            ...
            if (nodeElem.getTagName().equals("instance_geometry")) {

                String geomURL = nodeElem.getAttribute("url");
                Transformable parent = nodeTransformableMap.get(nodeElem.getParentNode().getParentNode());
                t = getGeometry(parseURL(geomURL), parent);
        ...
        } catch (ModelLoaderRegistry.LoaderException ex) {
    ...
    private Matrix parseMatrix(Element matrixElem) throws ModelLoaderRegistry.LoaderException {
        ...
        return new Matrix(matrixElem.getAttribute("sid"),
                toMinecraftCoords(matrix));
    }

    public Geometry getGeometry(String id, Transformable parent) {
        Element geomElem = null;
        try {
            geomElem = getXPathElement(String.format(
                    "library_geometries/geometry[@id='%s']", id));
            return parseGeometry(geomElem, parent);
        } catch (ModelLoaderRegistry.LoaderException e) {
            throw new RuntimeException(e);
        }
    }

    private Geometry parseGeometry(Element geomElem, Transformable parent) throws ModelLoaderRegistry.LoaderException {
        Geometry geom = new Geometry(parent);
        ...
        if (stride == 1) {
            ...
            if (paramElem.getAttribute("type").equals("float4x4")) {
                ...
                for (int j = 0; j < count; j++) {
                    System.arraycopy(float_data, j * 16, float4x4_data[j], 0, 16);
